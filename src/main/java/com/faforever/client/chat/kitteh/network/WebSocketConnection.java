...
public final class WebSocketConnection implements ClientConnection {
  ...
  private void scheduleReconnect(int delay) {
    Mono.delay(Duration.ofMillis(delay)).doOnNext(unused -> this.client.connect()).subscribe();
  }
  ...
  public void startPing() {
    this.ping = Flux.interval(Duration.ofSeconds(60)).doOnNext(unused -> this.client.ping()).subscribe();
  }
